...
# 2. เรียกใช้ KNN Imputer
knn_imputer = KNNImputer(n_neighbors=5)

# 3. Fit และ Transform เติมทับลงไป
df[cols_to_impute] = knn_imputer.fit_transform(df[cols_to_impute])

# ส่วนคอลัมน์ 0-1 (One-Hot) ถ้ามีค่าว่าง (NaN) ให้เติมด้วย Mode (0) แยกต่างหาก
dummy_cols = [col for col in df.columns if col not in cols_to_impute and df[col].dtype in ['float64', 'int64']]
df[dummy_cols] = df[dummy_cols].fillna(0)

# แปลงค่า binary features ให้กลับเป็น 0/1 โดยใช้เงื่อนไข >= 0.5
# คอลัมน์ที่เป็น binary features (yes/no questions)
binary_cols = ['Try_New_Flavor', 'Like_Stronger_Ebisen_Flavor', 'Believe_Ebisen_Shrimp']
if 'Believe_Ebisen_Shrimp_2' in df.columns:
    binary_cols.append('Believe_Ebisen_Shrimp_2')

# เพิ่มคอลัมน์ Is_Calvora_* ทั้งหมด
binary_cols.extend([col for col in df.columns if col.startswith('Is_Calvora_')])

# เพิ่มคอลัมน์อื่นๆ ที่เป็น binary
binary_cols.extend(['Calvora_Tagline_Awareness'])

# แปลงค่า >= 0.5 เป็น 1 (มีแนวโน้มสนใจ/ใช่), < 0.5 เป็น 0 (ไม่สนใจ/ไม่ใช่)
